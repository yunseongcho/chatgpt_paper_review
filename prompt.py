prompts_korean = {
    "Start": """당신은 인공지능 분야의 전문가이자 제공한 논문의 1저자입니다. 당신의 역할은 제공한 논문의 내용을 철저히 검토하고, 내가 던지는 질문에 대해, 제공된 논문을 기반으로 인공지능을 전공하고 있는 대학원생에게 답변하는 것입니다. 모든 답변은 논문을 기반으로 작성되어야 합니다. 거짓으로 답변하지 말고 모르면 모른다고 답변해주세요.
...
A. Author, B. Author, and C. Author, "Title of the paper," in Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), Year""",
    ...
    "Introduction": {
        "Background": "제공된 논문의 Introduction 부분을 면밀히 검토하여, 무엇이 이 연구를 시작하게한 주요한 동기(motivation) 이었는지 어떤 문제(problem) 나 도전(challenge) 을 해결하려고 했는지 설명해주십시오.",
        "Contribution": "제공된 논문을 Introduction 부분을 면밀히 검토하여, 이 논문의 주요 기여 (contribution) 에 대한 풍부하고 상세한 설명을 제공해 주실 수 있을까요?"
    },
    
    "Related Works": {
        "Related Works": """제공된 논문의 Related Work 부분을 면밀히 검토하여, 논문에서 다루고 있는 Task 혹은 Method 와 밀접한 연관이 있는 핵심 연구들을 소개해주실 수 있나요? 각 핵심 연구들을 논문에서 나눈 분류에 따라 분류해주십시오. 논문의 내용을 기반으로 답변해주십시오.
...
A. Author, B. Author, and C. Author, "Title of the paper," in Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), Year""",
        ...
        "Baseline": """제공된 논문을 면밀히 검토하여, 논문에서 설명하고 있는 방법론의 기반이 되는 Baseline 은 무엇인지, 논문의 Baseline은 어떤 구조 혹은 논문을 참고했는지 상세하고 풍부하게 알려주시면 대단히 감사하겠습니다. Baseline이 없다면 없다고 말씀해주세요.
...
A. Author, B. Author, and C. Author, "Title of the paper," in Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), Year""",
    ...
    "Methodology": {
        "Preliminaries": """제공된 논문을 면밀히 검토하여, 방법론(Method) 을 완전히 이해하기 위해 꼭 알아두어야 할 중요한 참고 자료나 선행 개념을 알려주시겠어요? 이러한 참고 자료나 개념을 간단하고 이해하기 쉽게 설명해 주시면 대단히 감사하겠습니다. 설명에 필요한 수식을 작성해주세요.
...
""",
        ...
        "Backbone": "제공된 논문의 방법론 부분을 면밀히 검토하여 논문의 내용을 바탕으로, 방법론의 Backbone network 는 무엇인지, 논문의 Backbone network 는 어떤 구조 혹은 논문을 참고했는지 상세하고 풍부하게 알려주시면 대단히 감사하겠습니다.",
        "Model Architecture": "제공된 논문의 방법론 부분을 면밀히 검토하여 논문의 내용을 바탕으로, 논문의 방법론의 모델 아키텍처에 대해 풍부하고 상세한 설명을 부탁드립니다. 답변이 길어져도 좋습니다.",
        "Training": "제공된 논문의 방법론 부분을 면밀히 검토하여 논문의 내용을 바탕으로, 논문의 훈련 방법에 대해 풍부하고 상세한 설명을 제공해 주실 수 있을까요? 단계적 프로세스로 세분화하여 명확하고 이해하기 쉽게 설명해 주시면 정말 감사하겠습니다. 답변이 길어져도 좋습니다.",
        "Loss function": "제공된 논문의 방법론 부분을 면밀히 검토하여 논문의 내용을 바탕으로, 이 논문에 사용된 손실 함수 (loss function) 또는 수식들에 대해 자세한 설명을 제공해 주실 수 있을까요? 각 손실 함수들이 Training의 어떤 단계에 쓰이는 지와 각 손실 함수들의 목적과 기능을 이해하기 쉽고 자세하게 설명해 주시기 바랍니다.",
        "Inference and Application": "제공된 논문을 면밀히 검토하여 논문의 내용을 바탕으로, 방법론 (Method) 의 Inference 또는 Application 방법에 있다면 상세하고 자세히 설명해주세요. Inference와 Application에 대한 부분이 명시되어있지 않다면 없다고 말해주세요. 논문의 내용에 기반하지 않은 답변은 안됩니다.",
        "Figure": "제공된 논문의 방법론 부분을 면밀히 검토하여, 방법론의 모델 아키텍처 또는 프레임워크, 추론 방법 등을 나타내는 그림의 위치를 논문을 기반으로 알려주시고, 그 그림에 대해 설명해준다면 감사하겠습니다. 질적평가 결과에 대한 그림은 제외시켜주세요. 논문의 내용에 기반하지 않은 답변은 안됩니다.",
        "Summary": "지금까지의 대화에서 방법론에 대해 논의한 사항을 고려하여 방법론에 대한 풍부하고 체계적인 요약을 작성해 주세요. 답변은 모델의 구조, 훈련 방법, 손실 함수, 추론 및 응용 방법 등을 파악하여 상세하고 포괄적으로 작성해야 합니다."
    },
    
    "Experiments": {
        "Datasets": "제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 이 논문에서 사용된 모든 데이터셋에 대한 종합적이고 상세한 목록을 제공해 주실 수 있을까요? 각 데이터셋의 특징과 역할을 구체적으로 설명해 주시면 매우 감사하겠습니다. 데이터셋의 특징으로는 어떠한 데이터를 모았고 그 양은 얼마나 되는지 등이 있으며 데이터셋의 역할에는 훈련(Training) 목적, 양적 평가, 응용(Application) 등이 있습니다.",
        "Implementation Details": "제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 훈련 (Training) 과정에서 사용된 구현 세부 사항 (implementation details) 이나 실험 설정에 대해 포괄적으로 설명해 주시겠어요? 특히 모델 훈련에 사용된 GPU의 수와 유형, 그리고 훈련 기간에 대해 알고 싶습니다. 또한 구체적인 하이퍼파라미터도 알려주세요. 첨부한 supplemental material도 참고 바랍니다.",
        "Quantitative Metrics": """제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 이 논문에서 사용된 구체적인 양적 지표 (quantitative metric) 들에 대해 수식 및 해석 방법등을 알려주십시오.
...
A. Author, B. Author, and C. Author, "Title of the paper," in Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR), Year""",
        ...
        "Quantitative Result": "제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 이 논문에서 제시된 방법론이 달성한 결과에 대한 종합적인 분석, 특히 앞서 언급한 데이터셋 (datasets) 및 양적 지표 (quantitative metrics) 관련된 분석을 제공해 주시겠습니까? 논문의 도표를 참고하여 이 방법론의 강점과 약점을 간략히 설명하고 이러한 결과가 도출된 이유에 대한 설명을 제공해 주시면 대단히 감사하겠습니다. 가능하다면 도표를 작성해주십시오. 첨부한 supplemental material도 참고 바랍니다. 모든 답변은 논문을 기반으로 작성되어야 합니다. 거짓으로 답변하지 말고 모르면 모른다고 답변해주세요.",
        "Qualitative Result": "제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 논문에서 제시된 질적 결과(qualitative result)에 대해 포괄적인 설명과 분석을 제공해 주실 수 있을까요? 또한, 이러한 질적 결과(qualitative result)를 완전히 이해하기 위해 몇 번째 그림과 섹션을 참조해야 하는지 구체적으로 알려주실 수 있을까요? 첨부한 supplemental material도 참고 바랍니다. 모든 답변은 논문을 기반으로 작성되어야 합니다. 거짓으로 답변하지 말고 모르면 모른다고 답변해주세요.",
        "Ablation Study": "제공된 논문의 실험 결과 및 해석 부분을 면밀히 검토하여, 논문에서 언급된 ablation study에 대해 포괄적인 설명을 제공해주시겠습니까? 연구의 목적, 방법론, 결과를 포함하여 ablation study에 대한 상세한 설명을 해주세요. 또한, ablation study 결과의 중요성과 의미를 명확히 알려주시기 바랍니다. 첨부한 supplemental material도 참고 바랍니다. 만약 ablation study가 없다면 없다고 말해주세요. 모든 답변은 논문을 기반으로 작성되어야 합니다. 거짓으로 답변하지 말고 모르면 모른다고 답변해주세요.",
        "Meaning of Result": "논문에서 제시된 결과의 의미와 중요성에 대해, 지금까지 답변한 모든 연구 결과와 그 시사점을 고려하여 철저하게 분석하고 해석한 상세하고 체계적인 설명을 제공해 주시겠습니까?"
    },
    
    "Conclusion": {
        "Limitation": "이 논문에서 제시한 방법의 한계에 대해 종합적이고 상세한 설명을 제공해주세요. 만약 논문에서 명시한 Limitation이 없다면 없다고 말해주세요. 모든 답변은 논문을 기반으로 작성되어야 합니다. 거짓으로 답변하지 말고 모르면 모른다고 답변해주세요.",
        # "Application": "앞서 언급한 한계를 어떻게 극복할 수 있을지, 이 방법론이 다른 AI/ML 방법론과 어떻게 결합될 수 있을 지에 대해 자세히 설명해 주세요. 인공지능 전문가로서 자유롭게 상상해주세요.",
        "Summary": "지금까지의 대화에서 논의한 사항을 고려하여 논문에 대한 풍부하고 체계적인 요약을 작성해 주세요. 답변은 논문의 주요 결과, 주장 및 기여를 파악하여 상세하고 포괄적으로 작성해야 합니다."
    },
